...
7) Add metadata (optional): `bash run_nifhdk_repro.sh --with-metadata` (requires the NCBI `datasets` CLI: e.g., `mamba install -c conda-forge ncbi-datasets-cli`, `conda install -c conda-forge ncbi-datasets-cli`, or `module load ncbi-datasets`)
